...
bool check_mission(Triangulation& tri, vector<pair<Point, long> >& j, vector<int>& component,
...
int evaluate(Triangulation& tri, vector<pair<Point, long> >& j, long p, vector<Point>& s, vector<Point>& t) {
    Graph G = build_graph(tri, p);
    ...
    for (int i = 0; i < m; i++) {
        bool cm = check_mission(tri, j, component, s[i], t[i], p);
        if ( cm ) {
            missions_completed |= (1<<i);
        }
    }
